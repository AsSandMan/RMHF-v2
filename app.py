...
# @app.route("/home_finance.json")
# @app.route("/.home_finance.json")
# def deny_json():
#     abort(403, "Access denied to data file")
...
#     if "localhost" in request.host or "127.0.0.1" in request.host:
#         # Временно возвращаем фейковый user_id для тестов
...
#     if not user_id:
#         abort(403, "Не удалось определить пользователя")

#     # Сохраняем user_id в объекте request для использования в маршрутах
...
#     """
#     user_id — Telegram ID пользователя (если передан — используем персональный файл)
#     """
#     if user_id:
#         # Персональный файл для каждого пользователя
#         filename = f"home_finance_{user_id}.json"
#     else:
#         filename = "home_finance.json"  # для обратной совместимости или тестов
...
#                 if key not in data.get("balance", {}):
#                     data["balance"][key] = 0
#             return data
#     except Exception as e:
...
#             save_data(data)  # сохраняем в персональный файл
#             return redirect(url_for("index"))
